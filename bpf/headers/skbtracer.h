...
struct event_t {
    char func_name[FUNCNAME_MAX_LEN]; // 4
    u64 skb;
    u64 start_ns;
    u8 flags;
    u8 pad[3];
...
// TODO effect
#define __sizeof_event_base 4+8+8+4+sizeof(struct pkt_info_t)+sizeof(struct l2_info_t)+sizeof(struct l3_info_t)+sizeof(struct l4_info_t)+sizeof(struct icmp_info_t)
...
set_event_info(struct sk_buff *skb, struct event_t *e)
{
    e->skb = (u64)skb;
...
set_pkt_info(struct sk_buff *skb, struct pkt_info_t *pkt_info)
{
	...
	bpf_get_current_comm(&pkt_info->comm, sizeof(pkt_info->comm));
...
set_ether_info(struct sk_buff *skb, struct l2_info_t *l2_info)
{
    ...
    bpf_probe_read(&l2_info->src_mac, 6, l2_header + 6); // src offset 6B
    bpf_probe_read(&l2_info->eth_proto, 2, l2_header + 12);

    l2_info->vlan_tci = BPF_CORE_READ(skb, vlan_tci) & 0xfff;
    l2_info->vlan_proto = BPF_CORE_READ(skb, vlan_proto);
    if (l2_info->eth_proto == bpf_htons(ETH_P_8021Q) || l2_info->eth_proto == bpf_htons(ETH_P_8021AD)) {
        l2_info->vlan_id = 8021;
    } else {
        l2_info->vlan_id = 0;
    }
//    bpf_skb_load_bytes(skb, 0, &(e->dst_mac),6);
//    bpf_skb_load_bytes(skb, 6, &(e->vlan_hdr),6);
//	  bpf_skb_load_bytes(skb, 12, &l2_info->eth_proto, 2);
}
